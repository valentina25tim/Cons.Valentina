...
#region Peremenyi
        ...
        public void Intro()
        {
            ...
            Console.WriteLine("Привет) сохранила визуально - поменяла частично внутренность. Чуть-чуть лучше стало и читабельнее.\nИграть можно, победитель будет) * клацать можно всё - не слетит, карты обнуляются после победы)" +
                "\nВывела на эк карту комп(маленькая фора и возможность проверить). Карта генерируестся, если что) \nЛогики называемых коорд программой нет - стреляет рандомно и, возможно, дважды в одну точку не бьет (но это не точно ;)");
            Console.Write("\nWelcome to " + name + "!\nInput your name -->");
